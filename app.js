...
function seedDefaultMenu(){
  ...
  const seeded = [
    ...
    { id: crypto.randomUUID(), name: "Poori", price: 35, imageUrl: "https://upload.wikimedia.org/wikipedia/commons/f/f4/Puri_Bhaji.jpg", isAvailable: true },
...
function saveItem(e){
  ...
  const imageUrl = $("#itemImageUrl").value.trim() || "assets/menu/placeholder.svg";
  const isAvailable = $("#itemAvailable").checked;
  if(!name || Number.isNaN(price)) return;
  const menu = getMenu();
  if(id){
    const it = menu.find(x=>x.id===id); if(!it) return;
    it.name = name; it.price = price; it.imageUrl = imageUrl; it.isAvailable = isAvailable;
  }else{
